...
from ..models.schemas import AntennaTestRequest
from ..services.antenna_tester import run_antenna_test
...
@router.post("/antenna-test")
async def start_antenna_test(req: AntennaTestRequest):
    test_id = f"ant-{int(time.time() * 1000)}"
    logger.info("Starting antenna test %s (label: %s, dual: %s)", test_id, req.label, req.dual_channel)
    ...
    def run():
        try:
            run_antenna_test(req.label, req.dual_channel, emit)
        except Exception as e:
            logger.error("Antenna test %s failed: %s", test_id, str(e))
            test = active_tests.get(test_id)
...
@router.get("/antenna-test/{test_id}/progress")
async def antenna_test_progress(test_id: str):
    test = active_tests.get(test_id)
    if not test:
